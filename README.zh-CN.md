...
**追踪 AI 研究前沿——实时资讯、每周方向简报，外加一个论文深读工作台。**

PicX 每小时聚合 AI 资讯，为每个跟踪方向每周出一期简报；每篇论文都配有 AI 相伴的全文阅读器，并可一键生成可视化白板图。你也可以上传自己的论文，走同一套管线精读对谈。
...
- **一键白板图**——把任意论文变成一张可视化白板图，可用内置模板或自定义提示词，在 `/settings/prompts` 管理。
- **BYOK**——每个账号每天都有白板图额度；在 `/settings/providers` 填入自带 API key（加密存储），生成时便不再占用额度。
