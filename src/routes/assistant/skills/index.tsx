...
import { SkillRow, SkillRowSkeleton } from "#/components/assistant/skill-row";
...
const skillSkeletonKeys = ["s1", "s2", "s3", "s4", "s5", "s6"];
...
function AssistantSkillsListPage() {
  ...
  const listBody = listQuery.isPending ? (
    <div>
      {skillSkeletonKeys.map((key) => (
        <SkillRowSkeleton key={key} />
      ))}
    </div>
  ) : listQuery.isError ? (
    ...
    <div className="rounded-2xl border border-dashed border-[var(--line)] px-6 py-16 text-center">
      <SearchX className="mx-auto size-8 text-[var(--ink-soft)] opacity-60" />
