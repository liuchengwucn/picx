...
/**
 * 两个聊天面板共用的 transport 工厂：服务端只收最后一条消息（历史真源在 D1），
 * 且 parts 仅放行 text。settingsRef 经 ref 取值——transport 用 useMemo 建一次，
 * 设置变化不重建。extraBody 提供路由定位字段（paperShortId / conversationId）；
